from RLGames.Chess import COLORS
from RLGames.gym_wrappers.GymPygameWrapper import PygameVideoRecorder
from RLGames.gym_wrappers.GymChess import GymChess
...
class ChessRobotFeatureExtractor(RobotFeatureExtractor):
    pass


class ChessNRobotFeatureExtractor(ChessRobotFeatureExtractor):
...
class ChessDRobotFeatureExtractor(ChessRobotFeatureExtractor):
...
class ChessTEFeatureExtractor(ChessRobotFeatureExtractor):
...
class ChessTemporalEvaluator(TemporalEvaluator):
    """Chess temporal evaluator for visit colors in a given order.
    ...
    def __init__(self, input_space, gamma=0.99, on_the_fly=False, relaxed=True):
        ...
        super().__init__(ChessTEFeatureExtractor(input_space),
...
chess_name2robot_feature_ext = {
    "N": ChessNRobotFeatureExtractor,
    "D": ChessDRobotFeatureExtractor
}

chess_name2temp_goals = {
    "colors": [ChessTemporalEvaluator],
    "colors_relaxed": [ChessTemporalEvaluator],
}


def _set_up_temporal_chess(config, args, env, robot_feature_extractor, brain):
    temporal_goals = []

    relaxed = False if args.temp_goal == "colors" else True if args.temp_goal == "colors_relaxed" else None
    temporal_goals.append(ChessTemporalEvaluator(env.observation_space,
...
def run_experiment(config:Config, args):
    ...
    if config.resume:
        ...
        robot_feature_extractor = chess_name2robot_feature_ext[args.robot_feature_space](env.observation_space)
        ...
        agent, trainer = _set_up_temporal_chess(config, args, env, robot_feature_extractor, brain)
